<template>
  <div v-show="visible">
    ...
    <el-row :gutter="20" class="equipInfobox">
      ...
      <el-col :xs="{span: 24}" :span="12">
        <label>设备序列号</label><span>{{infoData.SerialCode}}</span>
      </el-col>
      <el-col :xs="{span: 24}" :span="12">
        <label>设备验证码</label><span>{{infoData.ValidateCode}}</span>
      </el-col>
      <el-col :xs="{span: 24}" :span="12">
        <label>软件版本号</label><span>{{infoData.SoftwareVersion}}</span>
      </el-col>
      <el-col :xs="{span: 24}" :span="12">
        <label>通道号</label><span>{{infoData.ChannelNo}}</span>
      </el-col>
    </el-row>
    <el-row :gutter="20" class="equipInfobox">
      <h6>附属信息</h6>
