<template>
    <div class="app-container">
        <el-form class="xl-query" :model="queryParams" ref="queryForm" :inline="true" label-width="120px">
            ...
            <el-form-item>
                <el-select v-model="queryParams.withdrawType" clearable>
                    <el-option label="提现类型" value="" />
            ...
            <el-form-item prop="postCode">
                <el-input placeholder="起始商家ID" v-model="queryParams.merStartId" clearable size="small" @keyup.enter.native="handleQuery" />
            </el-form-item>
            <el-form-item prop="postCode">
                <el-input placeholder="终止商家ID" v-model="queryParams.merEndId" clearable size="small" @keyup.enter.native="handleQuery" />
            </el-form-item>
            <el-form-item prop="postCode">
                <el-input placeholder="商家ID" v-model="queryParams.merId" clearable size="small" @keyup.enter.native="handleQuery" />
            </el-form-item>
            <el-form-item prop="postCode">
                <el-input placeholder="手机号" v-model="queryParams.merAccount" clearable size="small" @keyup.enter.native="handleQuery" />
            </el-form-item>
            <el-form-item>
                <el-button :loading="loading" type="primary" icon="el-icon-search" size="mini" @click="handleQuery">搜索</el-button>
            </el-form-item>
            <el-form-item>
                <el-button :loading="loading" v-hasPermi="['system:withdraw:merchant:auditOne']" type="success" icon="el-icon-search" size="mini" @click="handleMutiplePass">批量审核</el-button>
            </el-form-item>
            <el-form-item>
                <el-button :loading="loading" v-hasPermi="['system:withdraw:merchant:auditTwo']" type="warning" icon="el-icon-search" size="mini" @click="handleMutiplePass1">批量兑现</el-button>
            </el-form-item>
            <!-- <el-button icon="el-icon-refresh" size="mini" @click="resetQuery">重置</el-button> -->
            <el-form-item>
                <el-button :loading="loading" v-hasPermi="['system:withdraw:merchant:excel']" type="warning" icon="el-icon-download" size="mini" @click="handleExport">导出</el-button>
            </el-form-item>
        </el-form>
        <el-table v-loading="listLoading" :data="postList" border @selection-change="handleSelection">
            ...
            <el-table-column width=" 100px" label="操作" align="center">
                <template slot-scope="{row,$index}">
                    <el-row>
                        <el-button v-hasPermi="['system:withdraw:merchant:auditOne']" v-if="row.auditState==1" type="success" :loading="loading" size="mini" @click="handlePass($index)">审核通过</el-button>
                        <el-button v-hasPermi="['system:withdraw:merchant:auditTwo']" v-if="row.auditState==2" type="success" :loading="loading" size="mini" @click="handlePass1($index)">兑现</el-button>
                    </el-row>
                    <el-row>
                        <el-button v-hasPermi="['system:withdraw:merchant:auditOne']" v-if="row.auditState==1||row.auditState==2" type="danger" :loading="loading" size="mini" @click="handleOpen('setSts',$index,0)">审核拒绝</el-button>
                    </el-row>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        handleOpenMerDetail(id) {
            const target = this.$refs.merDetail;
