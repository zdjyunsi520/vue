    ...
    <el-dialog width="800px" :title="'修改会员信息'" :visible.sync="dialogVisible" :modal-append-to-body="false" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" center :destroy-on-close="true">
        <el-form ref="form" :model="form" :rules="rules">
            ...
            <el-form-item label="开户行">
                <el-select v-model="form.bbuyerBindBank.bankCode" clearable>
                    <el-option label="所属银行" value="" />
                    <el-option v-for="item in bankType" :label="item.value" :value="item.key" />
                </el-select>
            </el-form-item>
            ...
            <el-form-item label=" 高佣金返款模式">
                <el-radio-group v-model="form.expensiveRefundMode">
                    <el-radio :label="2">货返</el-radio>
...
export default {
    ...
    computed: {
        ...mapGetters({
            bankType: "status/sys_bank_nameType",
            bankKV: "status/sys_bank_nameKV"
        })
    },
    methods: {
        getInfo() {
        ...
        handleSubmit() {
            this.$refs.form.validate(v => {
                if (v) {
                    this.loading = true;
                    var o = update;
                    this.form.bbuyerBindBank.bankName = this.bankKV[
                        this.form.bbuyerBindBank.bankCode
                    ];
                    o(this.form)
                        .then(res => {
                            this.$emit("getList");
