<template>
  <div class="app-container">
    <el-form ref="queryForm" :inline="true" label-width="120px" class="xl-query">
      ...
      <el-form-item>
        <el-input placeholder="拼多多账号" v-model="queryParams.pddAccount" clearable size="small" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        ...
        <el-button v-hasPermi="['system:account:Pddinfo:excel']" :loading="loading" type="primary" icon="el-icon-search" size="mini" @click="handleExport">导出</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table border v-loading="listLoading" :data="postList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="拼多多账号" align="center" prop="pddAccount" />
      ...
      <el-table-column label="个人中心图" align="center" prop="accountUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
            <img @click="handleShow(row.accountUrl)" :src="path+row.accountUrl+process" alt="">
          </el-row>
        </template>
      </el-table-column>
      <el-table-column label="收货地址图" align="center" prop="addressUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
      ...
      <el-table-column label="微信账号图" align="center" prop="wxAccountUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
            <img @click="handleShow(row.wxAccountUrl)" :src="path+row.wxAccountUrl+process" alt="">
          </el-row>
        </template>
      </el-table-column>
      ...
      <el-table-column width="110px" label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}" class="xl-table-column">
          <div v-if="row.auditState==0">
            <el-row>
              <el-button v-hasPermi="['system:account:Pddinfo:quality']" type="success" :loading="loading" size="small" icon="el-icon-edit" @click="handleReview($index,1)">审核优质</el-button>
            </el-row>
            <el-row>
              <el-button v-hasPermi="['system:account:Pddinfo:quality']" type="primary" :loading="loading" size="small" icon="el-icon-edit" @click="handleReview($index,0)">审核普通</el-button>
            </el-row>
            <el-row>
              <el-button v-hasPermi="['system:account:Pddinfo:turnDown']" type="danger" :loading="loading" size="small" icon="el-icon-edit" @click="handleReview($index,2)">审核驳回</el-button>
            </el-row>
            <el-row>
              <el-button v-hasPermi="['system:account:Pddinfo:pullBlack']" type="danger" :loading="loading" size="small" icon="el-icon-delete" @click="handleReview($index,3)">旺旺拉黑</el-button>
            </el-row>
          </div>
          <div v-else-if="row.isBlack == 0">
            <el-row>
              <el-button v-hasPermi="['system:account:Pddinfo:update']" type="warning" :loading="loading" size="small" icon="el-icon-edit" @click="handleOpen('update',$index)">修改</el-button>
            </el-row>
    ...
    <update ref="update" :platform="2" @getList="getList"></update>
  </div>
</template>
...
const reason = [
  ...
  { id: 2, title: '收货地址截图错误' },
  { id: 3, title: '微信账号截图错误' },
  { id: 4, title: '会员名与截图不一致' },
  { id: 5, title: '拼多多号不符合要求' },
...
export default {
  ...
  computed: {
    ...mapGetters({ accountType: 'status/accountType', path: 'imgOSS/path',
     reason:'status/pdd_wang_wang_refuse_remarksType'
    }),
    accountKV() {
