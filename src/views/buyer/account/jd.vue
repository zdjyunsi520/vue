<template>
  <div class="app-container">
    <el-form ref="queryForm" :inline="true" label-width="120px" class="xl-query">
      ...
      <el-form-item>
        <el-input placeholder="京东账号" v-model="queryParams.jdAccount" clearable size="small" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        ...
        <el-button v-hasPermi="['system:account:Jdinfo:excel']" :loading="loading" type="primary" icon="el-icon-search" size="mini" @click="handleExport">导出</el-button>
      </el-form-item>
    </el-form>
    <el-table border v-loading="listLoading" :data="postList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="收货地址截图" align="center" prop="jdAccountUrl">
      ...
      <el-table-column label="京东账号截图" align="center" prop="jdAccountUrl">
        <template slot-scope="{row,$index}">
    
          <el-row class="xl-row-img">
            <img @click="handleShow(row.jdAccountUrl)" :src="path+row.jdAccountUrl+process" alt="">
          </el-row>
        </template>
      </el-table-column>
      <el-table-column label="实名认证" align="center" prop="realNameUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
      ...
      <!-- <el-table-column label="账号等级" align="center" prop="levUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
            <img @click="handleShow(row.levUrl)" :src="path+row.levUrl" alt="">
          </el-row>
        </template>
      </el-table-column>
      <el-table-column label="信誉等级" align="center" prop="jdCreditUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
            <img @click="handleShow(row.jdCreditUrl)" :src="path+row.jdCreditUrl" alt="">
          </el-row>
        </template>
      </el-table-column>
      <el-table-column label="京东白条" align="center" prop="jdWhiteUrl">
        <template slot-scope="{row,$index}">
          <el-row class="xl-row-img">
            <img @click="handleShow(row.jdWhiteUrl)" :src="path+row.jdWhiteUrl" alt="">
          </el-row>
        </template>
      </el-table-column> -->
      ...
      <el-table-column label="审核理由" align="center">
        <template slot-scope="{row}">
          ...
          <el-input type="textarea" :rows="5" v-if="row.note123 == ''" v-model="row.note"></el-input>
        </template>
      </el-table-column>
      <el-table-column width="120px" label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}" class="xl-table-column">
          ...
          <div v-if="row.auditState==0">
            <el-row>
              <el-button  v-hasPermi="['system:account:Jdinfo:quality']" type="success" :loading="loading" icon="el-icon-edit" @click="handleReview($index,1)">审核优质</el-button>
            </el-row>
            <el-row>
              <el-button v-hasPermi="['system:account:Jdinfo:quality']" type="primary" :loading="loading" icon="el-icon-edit" @click="handleReview($index,0)">审核普通</el-button>
            </el-row>
            <el-row>
              <el-button v-hasPermi="['system:account:Jdinfo:turnDown']" type="danger" :loading="loading" icon="el-icon-edit" @click="handleReview($index,2)">审核驳回</el-button>
            </el-row>
            <el-row>
              <el-button v-hasPermi="['system:account:Jdinfo:pullBlack']" type="info" :loading="loading" icon="el-icon-delete" @click="handleReview($index,3)">旺旺拉黑</el-button>
            </el-row>
          </div>
          <div v-else-if="row.isBlack == 0">
            <el-row>
                <el-button v-hasPermi="['system:account:Jdinfo:update']" type="warning" :loading="loading"  icon="el-icon-edit" @click="handleOpen('update',$index)">修改</el-button>
             </el-row>
    ...
    <update ref="update" :platform="3" @getList="getList"></update>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    reason:'status/jd_wang_wang_refuse_remarksType'
    }),
    accountKV() {
